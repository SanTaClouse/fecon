// (Re)inicializa la base Neon: recrea la tabla `catalogo` con el esquema actual
// y la siembra con el catálogo de ejemplo (SEED).
//
// Uso (en local):  npx tsx scripts/db-seed.ts   (o  npm run db:seed)
//
// Lee la connection string de .env.local (DATABASE_URL_UNPOOLED o DATABASE_URL).
// OJO: recrea la tabla (DROP + CREATE), así que reemplaza lo que haya.

import { readFileSync } from "node:fs";
import { neon } from "@neondatabase/serverless";
import { SEED } from "../lib/presupuestos/catalog";

try {
  const env = readFileSync(new URL("../.env.local", import.meta.url), "utf8");
  for (const line of env.split(/\r?\n/)) {
    const m = line.match(/^\s*([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(.*?)\s*$/);
    if (!m) continue;
    let v = m[2];
    if (
      (v.startsWith('"') && v.endsWith('"')) ||
      (v.startsWith("'") && v.endsWith("'"))
    )
      v = v.slice(1, -1);
    if (process.env[m[1]] === undefined) process.env[m[1]] = v;
  }
} catch {
  console.warn("No pude leer .env.local; uso las variables del entorno.");
}

const url = process.env.DATABASE_URL_UNPOOLED || process.env.DATABASE_URL;
if (!url) {
  console.error("✗ Falta DATABASE_URL / DATABASE_URL_UNPOOLED en .env.local");
  process.exit(1);
}

const sql = neon(url);

async function main() {
  await sql.transaction([
    sql`DROP TABLE IF EXISTS catalogo`,
    sql`
      CREATE TABLE catalogo (
        id BIGSERIAL PRIMARY KEY,
        tipo TEXT NOT NULL,
        categoria TEXT NOT NULL,
        item TEXT NOT NULL,
        unidad TEXT NOT NULL DEFAULT 'u',
        cantidad NUMERIC NOT NULL DEFAULT 1,
        materiales NUMERIC NOT NULL DEFAULT 0,
        mano_obra NUMERIC NOT NULL DEFAULT 0,
        orden INTEGER NOT NULL DEFAULT 0,
        actualizado_en TIMESTAMPTZ NOT NULL DEFAULT now()
      )
    `,
    ...SEED.map(
      (it, i) => sql`
        INSERT INTO catalogo
          (tipo, categoria, item, unidad, cantidad, materiales, mano_obra, orden)
        VALUES
          (${it.tipo}, ${it.categoria}, ${it.item}, ${it.unidad},
           ${it.cantidad}, ${it.materiales}, ${it.manoObra}, ${i})
      `
    ),
  ]);
  console.log(`✓ Tabla recreada y sembrada con ${SEED.length} ítems en Neon.`);
}

main()
  .then(() => process.exit(0))
  .catch((err) => {
    console.error("✗ Error sembrando la base:", err);
    process.exit(1);
  });
